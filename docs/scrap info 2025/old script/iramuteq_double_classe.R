...
# -------------------------------------------------------------------------
# 7) Charger les données RDS (issues de l'étape précédente)
# -------------------------------------------------------------------------
...
# -------------------------------------------------------------------------
# 8) Fusionner les classes
# -------------------------------------------------------------------------

df_segment <- df_segment_global %>%
  left_join(df_segment_local %>%
              select(video_id,id_segment,classe_local = classe))

stopifnot(nrow(df_segment) == nrow(df_segment_global))
stopifnot(df_segment %>% filter(is.na(classe)) %>% nrow() == 0)
stopifnot(df_segment %>% filter(is.na(classe_local)) %>% nrow() == 0)

# -------------------------------------------------------------------------
# 9) Correction manuelle des classes
# -------------------------------------------------------------------------

# Il y a quelques cas où la classe local n'a pas de sens avec la classe global
# 1 Société   Justice          1
# 2 Société   Religion         5
...
df_segment %>% filter(classe == "Politique",classe_local == "Autre") %>% pull(text)

# Je me rends compte que les 86 Autre local et Politique global
# ça semble souvent être des débuts d'interview

df_segment %>% filter(classe == "Politique",classe_local == "Autre") %>%
  count(id_segment)

# Oui, en effet : faisons simple : tous en Autre local et global
df_segment <- df_segment %>%
  mutate(
    classe = case_when(
      classe == "Politique" & classe_local == "Autre" ~ "Autre",
      TRUE ~ classe)
  )

df_segment %>% count(classe,classe_local) %>% arrange(n) %>% print(n=100)

# Identifier l'id de la classe
df_segment <- df_segment %>%
  mutate(
...
# -------------------------------------------------------------------------
# 10) Sauvegarder df commune dans data
# -------------------------------------------------------------------------
